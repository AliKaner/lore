    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black">
      ...
      <div className="max-w-5xl mx-auto px-4 py-16">
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-8 mb-8">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            ...
            <div className="flex-1 space-y-4">
              <div>
                <h1 className="text-4xl font-bold text-white font-title mb-2">
                  {book.title}
                </h1>
                {book.universe && (
                  <p className="text-gray-400 font-text mb-3">
                    {book.universe.name}
                  </p>
                )}
                {book.description && (
              ...
              {chapters && chapters.length > 0 && (
                <div className="flex gap-4">
                  <Link
                    href={`/book/${bookId}/${chapters[0]._id}`}
                  ...
                  {chapters.length > 1 && (
                    <Link
                      href={`/book/${bookId}/${chapters[chapters.length - 1]._id}`}
                      className="px-6 py-3 bg-transparent border border-white/50 rounded-lg text-white font-semibold hover:bg-white/10 transition-all duration-300"
                    >
                      Read Last Chapter
                    </Link>
                  )}
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-8">
          <h2 className="text-2xl font-bold text-white mb-6 font-title">
            Chapters
          </h2>
          {chapters.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters.map((chapter, index) => (
                <Link
                  key={chapter._id}
                  href={`/book/${bookId}/${chapter._id}`}
                  className="group bg-white/5 border border-white/10 rounded-lg p-4 hover:bg-white/10 hover:border-white/20 transition-all duration-300"
                >
